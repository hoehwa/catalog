...
Surely, this dealt with encoding, but the multiline string has sneaked a newline character () into the URL. This is because template literals preserve whitespace, leading to such issues. Breaking the string into multiple lines and concatenating them can help, but the code is starting to get ugly.
...
While query parameters are the most common issue when dealing with URLs, the `URL` object can be useful in many other situations. For example, you can change the protocol, hostname, port, path, hash, etc. of a URL, or even parse an existing URL into its components. If you're interested, you should check out previous articles about how you can [edit URL parameters](https://github.com/mindulle/Documents/blob/main/js/s/edit-url-params/README.md) and the [`window.location` cheat sheet](https://github.com/mindulle/Documents/blob/main/js/s/window-location-cheatsheet/README.md).
